...
def print_gpu_memory_usage():
    ...
    for i in range(num_gpus):
        ...
        print(f"GPU {i} memory in total: {torch.cuda.get_device_properties(i).total_memory / 1024 ** 3:.2f} GB")
...
def initialize_pipeline(model_config_dict):
    ...
    model_path = os.path.join(model_config_dict["CURRENT_PROJECT"], model_config_dict["USER_FOLDER"],"model", model)
    if not model_path:
    ...
    generation_config.max_length = 512  # This is a max length for every model which can be the same
    ...
    dynamic_text_pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer,device_map="auto")
...
## llama2 7b 13b 70b usable
## mixtral 7b, 8*7b, 8*22b () usable
## llma3 8b, 70b usable
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_path', type=str, help='paht to the current experiment input folder')
    ...
    exp_df = read_csv_prompt_from_folder(INPUT_PROMPT_PATH)  # This function tries to find the csv file inside the folder 
    ...
    for index, row in exp_df.iterrows():
        ...
        prompt = match_prompt_column(CURRENT_RUNNING_MODEL, row) # Find the corresponding prompt
        start_time = time.time()
        temperature = float(row["Temperature"])
        updated_row["generated_response"] = generate_text(pipeline=pipeline, prompt=prompt, temperature =temperature)
        end_time = time.time()
        ...
        if times == 0:
            updated_dataframe.to_csv(f"{INPUT_PROMPT_PATH}/{output_file_name}", index=False, mode="w", header=True)
        else:
            updated_dataframe.to_csv(f"{INPUT_PROMPT_PATH}/{output_file_name}", index=False, mode="a", header=False)
        pbar.update(1)
        times = times + 1

...
def main(args):
    ...
    if config.get("is_new_file", False):
        output_file = f"{PREFIX}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_reasoning.jsonl"
        start_idx= 0
    else:
        if latest_file:
    ...
    for i, row in tqdm(
        ...
        generated_message = generate_text_with_vllm(config, updated_row["input"])
        generated_text = generated_message["content"]
        generated_reasoning_text = generated_message["reasoning_content"]
        updated_row["generate_response"] = generated_text
        updated_row["generate_reasoning_response"] = generated_reasoning_text
...
# python script/vllm_running_inference_reasoning_model.py --model_name "DeepSeek-R1-Distill-Qwen-1.5B" --model_path_base "/home/snt/llm_models/" --input_path_folder "/home/snt/projects_lujun/temperature_eval/data/Intermediate/" --rep 3 --server_url "http://0.0.0.0:8000/v1/chat/completions" --is_new_file
